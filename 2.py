...
def save_page_as_pdf(html_content):
    ...
    options = {
        'no-images': '',  # Esto omitirá las imágenes si están causando problemas
        'no-outline': None,  # Evita problemas con la generación de contornos en el PDF
        'disable-smart-shrinking': '',  # Evita que wkhtmltopdf reduzca el tamaño del contenido
        'custom-header': [('Accept-Encoding', 'gzip')]  # Ajuste en los encabezados de red
    }
    try:
